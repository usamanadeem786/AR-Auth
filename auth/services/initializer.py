...
from auth.db import AsyncEngine, AsyncSession
...
class Initializer:
    ...
    async def _init_default_client(
        ...
        redirect_uris = [
            f"http://{self.settings.auth_domain}/admin/auth/callback",
            f"https://{self.settings.auth_domain}/admin/auth/callback",
            f"http://{self.settings.auth_domain}/docs/oauth2-redirect",
            f"https://{self.settings.auth_domain}/docs/oauth2-redirect",
        ]
        ...
        client.client_secret = self.settings.auth_client_secret
        if self.settings.auth_encryption_key is not None:
            client.encrypt_jwk = self.settings.auth_encryption_key
