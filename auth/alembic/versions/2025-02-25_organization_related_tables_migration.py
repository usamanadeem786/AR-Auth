...
def upgrade():
    ...
    if dialect == "postgresql":
        organization_role_enum = postgresql.ENUM(
            ...
            "ADMIN",
            "MEMBER",
            name=f"{table_prefix}organizationrole",
            create_type=False,
        )
        organization_role_enum.create(connection, checkfirst=True)
    else:
        organization_role_enum = sa.Enum(
            "OWNER", "ADMIN", "MEMBER", name=f"{table_prefix}organizationrole"
        )
    ...
    op.create_index(
        op.f(f"ix_{table_prefix}organization_invitations_role"),
        f"{table_prefix}organization_invitations",
        ["role"],
        unique=False,
    )
    op.create_index(
        op.f(f"ix_{table_prefix}organization_invitations_token"),
        f"{table_prefix}organization_invitations",
        ["token"],
        unique=True,
    )
